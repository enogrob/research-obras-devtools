...
class SubstituteCommand extends node.CommandBase {
    ...
    async replaceTextAtLine(line, regex, vimState) {
        ...
        if (this._arguments.flags & SubstituteFlags.ConfirmEach) {
            ...
            for (const match of matches) {
                ...
                if (!(this._arguments.flags & SubstituteFlags.ConfirmEach) ||
                    ...
                    await textEditor_1.TextEditor.replace(new vscode.Range(line, 0, line, rangeEnd), newContent);
            ...
            await textEditor_1.TextEditor.replace(new vscode.Range(line, 0, line, originalContent.length), originalContent.replace(regex, this._arguments.replace));
            globalState_1.globalState.jumpTracker.recordJump(new jump_1.Jump({
                editor: vimState.editor,
                fileName: vimState.editor.document.fileName,
                position: new position_1.Position(line, 0),
    ...
    async confirmReplacement(replacement, line, vimState, match, matchIndex) {
