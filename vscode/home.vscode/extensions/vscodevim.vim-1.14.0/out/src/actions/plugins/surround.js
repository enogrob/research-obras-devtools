...
let CommandSurroundAddToReplacement = CommandSurroundAddToReplacement_1 = class CommandSurroundAddToReplacement extends actions_1.BaseCommand {
    ...
    async exec(position, vimState) {
        ...
        if (await CommandSurroundAddToReplacement_1.TryToExecuteSurround(vimState, position)) {
            this.isCompleteAction = true;
        }
        return vimState;
    }
    static async Finish(vimState) {
    ...
    static async TryToExecuteSurround(vimState, position) {
        ...
        if (target === 't') {
            ...
            let innerTagContent = await new motion_1.MoveInsideTag().execAction(position, vimState);
            let { start, stop, failed } = await new motion_1.MoveAroundTag().execAction(position, vimState);
