...
let SelectWord = class SelectWord extends TextObjectMovement {
    ...
    async execAction(position, vimState) {
        ...
        if (/\s/.test(currentChar)) {
            ...
            if (stop.isEqual(textEditor_1.TextEditor.getFirstNonWhitespaceCharOnLine(stop.line))) {
            ...
            if (stop.isEqual(position.getCurrentWordEnd(true)) &&
                !position
                    .getWordLeft(true)
                    .isEqual(textEditor_1.TextEditor.getFirstNonWhitespaceCharOnLine(stop.line)) &&
...
let SelectABigWord = class SelectABigWord extends TextObjectMovement {
    ...
    async execAction(position, vimState) {
        ...
        if (/\s/.test(currentChar)) {
            ...
            else if ((nextWord.isEqual(textEditor_1.TextEditor.getFirstNonWhitespaceCharOnLine(nextWord.line)) ||
                nextWord.isLineEnd()) &&
                vimState.recordedState.count === 0) {
                start = position.getLastWordEnd().getRight();
                stop = position.getLineEnd();
...
let SelectAnExpandingBlock = class SelectAnExpandingBlock extends motion_1.ExpandingSelection {
    ...
    async execAction(position, vimState) {
        ...
        for (const iMotion of ranges) {
            ...
            const range = new range_1.Range(iMotion.start, iMotion.stop);
...
let SelectParagraph = class SelectParagraph extends TextObjectMovement {
    ...
    async execAction(position, vimState) {
        ...
        if (textEditor_1.TextEditor.getLineAt(position).isEmptyOrWhitespace) {
        ...
        while (stop.line < textEditor_1.TextEditor.getLineCount() - 1 &&
            textEditor_1.TextEditor.getLineAt(start.getDown()).isEmptyOrWhitespace) {
...
let SelectInnerParagraph = class SelectInnerParagraph extends TextObjectMovement {
    ...
    async execAction(position, vimState) {
        ...
        if (textEditor_1.TextEditor.getLineAt(position).isEmptyOrWhitespace) {
            ...
            stop = position.getLineEnd();
            while (start.line > 0 && textEditor_1.TextEditor.getLineAt(start.getUp()).isEmptyOrWhitespace) {
            ...
            if (textEditor_1.TextEditor.getLineAt(currentParagraphBegin).isEmptyOrWhitespace) {
                start = currentParagraphBegin.getRightThroughLineBreaks();
            }
            else {
                start = currentParagraphBegin;
            ...
            while (stop.line > 0 && textEditor_1.TextEditor.getLineAt(stop).isEmptyOrWhitespace) {
...
let SelectEntire = class SelectEntire extends TextObjectMovement {
    constructor() {
        super(...arguments);
        this.keys = ['a', 'e'];
    }
    async execAction(position, vimState) {
        return {
            ...
            stop: textEditor_1.TextEditor.getDocumentEnd(),
        };
    }
};
SelectEntire = __decorate([
    base_1.RegisterAction
], SelectEntire);
exports.SelectEntire = SelectEntire;
let SelectEntireIgnoringLeadingTrailing = class SelectEntireIgnoringLeadingTrailing extends TextObjectMovement {
    constructor() {
        super(...arguments);
        this.keys = ['i', 'e'];
    }
    async execAction(position, vimState) {
        ...
        let stop = textEditor_1.TextEditor.getDocumentEnd();
        while (start.line < stop.line && textEditor_1.TextEditor.getLineAt(start).isEmptyOrWhitespace) {
            start = start.getDown();
        }
        while (stop.line > start.line && textEditor_1.TextEditor.getLineAt(stop).isEmptyOrWhitespace) {
            stop = stop.getUp();
        }
...
SelectEntireIgnoringLeadingTrailing = __decorate([
    base_1.RegisterAction
...
class IndentObjectMatch extends TextObjectMovement {
    ...
    async execAction(position, vimState) {
        ...
        const firstValidLine = textEditor_1.TextEditor.getLine(firstValidLineNumber);
        ...
        if (isChangeOperator) {
            startCharacter = textEditor_1.TextEditor.getLine(startLineNumber).firstNonWhitespaceCharacterIndex;
        }
    ...
    static findFirstValidLine(cursorPosition) {
        for (let i = cursorPosition.line; i >= 0; i--) {
            if (!textEditor_1.TextEditor.getLine(i).isEmptyOrWhitespace) {
                return i;
            }
        }
