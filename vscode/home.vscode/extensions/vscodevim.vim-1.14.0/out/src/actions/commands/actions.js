    ...
    async exec(position, vimState) {
        ...
        for (const contentChange of this.contentChanges) {
            ...
            if (newStart.isEqual(newEnd)) {
                await textEditor_1.TextEditor.insert(textDiff.text, position_1.Position.FromVSCodePosition(newStart));
            }
            else {
                await textEditor_1.TextEditor.replace(vimState.editor.selection, textDiff.text);
            }
        }
        /**
...
let CommandQuitRecordMacro = class CommandQuitRecordMacro extends BaseCommand {
    ...
    async exec(position, vimState) {
        let existingMacro = (await register_1.Register.getByKey(vimState.recordedMacro.registerName))
            .text;
    ...
    async exec(position, vimState) {
        ...
        if (vimState.currentMode === mode_1.Mode.VisualLine) {
            ...
            vimState.cursors = vimState.cursors.map((cursor) => {
                ...
                return new range_1.Range(pos, pos);
            });
        }
        await vimState.setCurrentMode(mode_1.Mode.Normal);
...
class CommandScrollAndMoveCursor extends BaseCommand {
    ...
    async exec(position, vimState) {
        ...
        const newPositionLine = util_1.clamp(position.line + (this.to === 'down' ? scrollLines : -scrollLines), 0, vimState.editor.document.lineCount - 1);
    ...
    async exec(position, vimState) {
        ...
        if (vimState.currentMode === mode_1.Mode.Visual) {
            text = vimState.cursors
                .map((range) => {
                const [start, stop] = position_1.Position.sorted(range.start, range.stop);
...
let PutCommand = PutCommand_1 = class PutCommand extends BaseCommand {
    ...
    static async getText(vimState, multicursorIndex = undefined) {
    ...
    async exec(position, vimState, after = false, adjustIndent = false) {
        const register = await register_1.Register.get(vimState);
        ...
        let text = await PutCommand_1.getText(vimState, this.multicursorIndex);
        let textToAdd;
        let whereToAddText;
    ...
    async execVisualBlockPaste(block, position, vimState, after) {
        if (after) {
        ...
        if (linesToAdd > 0) {
            await textEditor_1.TextEditor.insertAt(Array(linesToAdd).join('\n'), new position_1.Position(textEditor_1.TextEditor.getLineCount() - 1, textEditor_1.TextEditor.getLineLength(textEditor_1.TextEditor.getLineCount() - 1)));
        }
        ...
        for (let lineIndex = position.line; lineIndex < position.line + block.length; lineIndex++) {
            ...
            const insertPos = new position_1.Position(lineIndex, Math.min(position.character, textEditor_1.TextEditor.getLineLength(lineIndex)));
            await textEditor_1.TextEditor.insertAt(line, insertPos);
        }
    ...
    async execCount(position, vimState) {
        ...
        if (vimState.effectiveRegisterMode === register_1.RegisterMode.LineWise &&
            ...
            const numNewlines = (await PutCommand_1.getText(vimState, this.multicursorIndex)).split('\n').length *
...
let PutCommandVisual = class PutCommandVisual extends BaseCommand {
    ...
    async exec(position, vimState, after = false) {
        ...
        let register = await register_1.Register.get(vimState);
        if (register.registerMode === register_1.RegisterMode.LineWise) {
            const replaceRegister = await register_1.Register.getByKey(vimState.recordedState.registerName);
            let deleteResult = await new operator.DeleteOperator(this.multicursorIndex).run(vimState, start, end, true);
            const deletedRegister = await register_1.Register.getByKey(vimState.recordedState.registerName);
        ...
        putResult = await new operator.DeleteOperator(this.multicursorIndex).run(putResult, start, end.getLeftIfEOL(), false);
...
let PutBeforeWithIndentCommand = class PutBeforeWithIndentCommand extends BaseCommand {
    ...
    async exec(position, vimState) {
        ...
        if (vimState.effectiveRegisterMode === register_1.RegisterMode.LineWise) {
            result.cursorStopPosition = textEditor_1.TextEditor.getFirstNonWhitespaceCharOnLine(result.cursorStopPosition.getUp().line);
        }
        return result;
    }
};
...
let CommandRepeatSubstitution = class CommandRepeatSubstitution extends BaseCommand {
    ...
    async exec(position, vimState) {
        ...
        await commandLine_1.commandLine.Run('s', vimState);
        return vimState;
    }
};
...
let CommandCenterScrollFirstChar = class CommandCenterScrollFirstChar extends BaseCommand {
    ...
    async exec(position, vimState) {
        ...
        vimState.cursorStopPosition = textEditor_1.TextEditor.getFirstNonWhitespaceCharOnLine(vimState.cursorStopPosition.line);
        return vimState;
    }
};
...
async function selectLastSearchWord(vimState, direction) {
...
let CommandVisualLineMode = class CommandVisualLineMode extends BaseCommand {
    ...
    async exec(position, vimState) {
        ...
        if (vimState.recordedState.count > 1) {
            vimState.cursorStopPosition = vimState.cursorStopPosition.getDown(vimState.recordedState.count - 1);
        }
        return vimState;
    }
};
...
let CommandInsertAtFirstCharacter = class CommandInsertAtFirstCharacter extends BaseCommand {
    ...
    async exec(position, vimState) {
        await vimState.setCurrentMode(mode_1.Mode.Insert);
        vimState.cursorStopPosition = textEditor_1.TextEditor.getFirstNonWhitespaceCharOnLine(position.line);
        return vimState;
    }
};
...
let ActionDeleteChar = class ActionDeleteChar extends BaseCommand {
    ...
    async exec(position, vimState) {
        ...
        let timesToRepeat = vimState.recordedState.count || 1;
        const state = await new operator.DeleteOperator(this.multicursorIndex).run(vimState, position, position.getRight(timesToRepeat - 1));
        await state.setCurrentMode(mode_1.Mode.Normal);
        return state;
    }
};
...
let ActionDeleteLastChar = class ActionDeleteLastChar extends BaseCommand {
    ...
    async exec(position, vimState) {
        ...
        let timesToRepeat = vimState.recordedState.count || 1;
        const state = await new operator.DeleteOperator(this.multicursorIndex).run(vimState, position.getLeft(timesToRepeat), position.getLeft());
        return state;
    }
};
...
let ActionJoin = class ActionJoin extends BaseCommand {
    ...
    async execJoinLines(startPosition, position, vimState, count) {
        ...
        for (let i = startLineNumber + 1; i <= endLineNumber; i++) {
            const lineText = textEditor_1.TextEditor.getLine(i).text;
        ...
        if (!deleteStartPosition.isEqual(deleteEndPosition)) {
            if (startPosition.isEqual(position)) {
                ...
                vimState.cursorStartPosition = vimState.cursorStopPosition = new position_1.Position(startPosition.line, trimmedLinesContent.length - columnDeltaOffset);
                await vimState.setCurrentMode(mode_1.Mode.Normal);
            }
        }
        return vimState;
...
let ActionJoinVisualBlockMode = class ActionJoinVisualBlockMode extends BaseCommand {
    constructor() {
        super(...arguments);
        this.modes = [mode_1.Mode.VisualBlock];
        this.keys = ['J'];
...
let ActionJoinNoWhitespace = class ActionJoinNoWhitespace extends BaseCommand {
    ...
    async exec(position, vimState) {
        ...
        lineTwo = lineTwo.substring(textEditor_1.TextEditor.getFirstNonWhitespaceCharOnLine(position.getDown().line).character);
        let resultLine = lineOne + lineTwo;
        let newState = await new operator.DeleteOperator(this.multicursorIndex).run(vimState, position.getLineBegin(), lineTwo.length > 0
...
let ActionReplaceCharacterVisual = class ActionReplaceCharacterVisual extends BaseCommand {
    ...
    async exec(position, vimState) {
        ...
        [start, end] = position_1.Position.sorted(start, end);
        ...
        for (let lineNum = start.line; lineNum <= end.line; lineNum++) {
            // Get line of text
            const lineText = textEditor_1.TextEditor.getLine(lineNum).text;
            if (start.line === end.line) {
...
let ActionGoToInsertVisualBlockMode = class ActionGoToInsertVisualBlockMode extends BaseCommand {
    ...
    async exec(position, vimState) {
        ...
        vimState.isMultiCursor = true;
        vimState.isFakeMultiCursor = true;
        for (const { line, start } of textEditor_1.TextEditor.iterateLinesInBlock(vimState)) {
...
let ActionChangeToEOLInVisualBlockMode = class ActionChangeToEOLInVisualBlockMode extends BaseCommand {
    ...
    async exec(position, vimState) {
        ...
        vimState.isMultiCursor = true;
        vimState.isFakeMultiCursor = true;
        for (const { end } of textEditor_1.TextEditor.iterateLinesInBlock(vimState)) {
...
class ActionGoToInsertVisualLineModeCommand extends BaseCommand {
    ...
    async exec(position, vimState) {
        ...
        for (const selection of vimState.editor.selections) {
            let start = position_1.Position.FromVSCodePosition(selection.start);
            let end = position_1.Position.FromVSCodePosition(selection.end);
            if (start.isAfter(end)) {
                [start, end] = [end, start];
            }
            for (let i = start.line; i <= end.line; i++) {
                const line = textEditor_1.TextEditor.getLine(i);
...
let ActionGoToInsertVisualBlockModeAppend = class ActionGoToInsertVisualBlockModeAppend extends BaseCommand {
    ...
    async exec(position, vimState) {
        ...
        vimState.isMultiCursor = true;
        vimState.isFakeMultiCursor = true;
        for (const { line, end } of textEditor_1.TextEditor.iterateLinesInBlock(vimState)) {
            if (line.trim() === '') {
                vimState.recordedState.transformations.push({
                    type: 'replaceText',
...
let ActionChangeChar = class ActionChangeChar extends BaseCommand {
    ...
    async exec(position, vimState) {
        ...
        await state.setCurrentMode(mode_1.Mode.Insert);
        return state;
    }
...
let ToggleCaseAndMoveForward = class ToggleCaseAndMoveForward extends BaseCommand {
    ...
    async exec(position, vimState) {
        await new operator.ToggleCaseOperator().run(vimState, vimState.cursorStopPosition, vimState.cursorStopPosition);
...
class IncrementDecrementNumberAction extends BaseCommand {
    ...
    async exec(position, vimState) {
        ...
        for (let { start, end, word } of textEditor_1.TextEditor.iterateWords(whereToStart)) {
            ...
            do {
                ...
                if (position.character <
                    ...
                    vimState.cursorStopPosition = await this.replaceNum(num, this.offset * (vimState.recordedState.count || 1), start, end);
    ...
    async replaceNum(start, offset, startPos, endPos) {
        ...
        if (oldWidth === newNum.length) {
            await textEditor_1.TextEditor.replace(range, newNum);
        }
        else {
            // Can't use replace, since new number is a different width than old
            await textEditor_1.TextEditor.delete(range);
            await textEditor_1.TextEditor.insertAt(newNum, startPos);
