...
let MarkMovementBOL = class MarkMovementBOL extends baseMotion_1.BaseMovement {
    ...
    async execAction(position, vimState) {
        ...
        return textEditor_1.TextEditor.getFirstNonWhitespaceCharOnLine(mark.position.line);
    }
};
...
async function ensureEditorIsActive(editor) {
...
let MoveDownNonBlank = class MoveDownNonBlank extends baseMotion_1.BaseMovement {
    ...
    async execActionWithCount(position, vimState, count) {
        vimState.currentRegisterMode = register_1.RegisterMode.LineWise;
        return textEditor_1.TextEditor.getFirstNonWhitespaceCharOnLine(position.getDown(Math.max(count, 1)).line);
    }
};
...
let MoveUpNonBlank = class MoveUpNonBlank extends baseMotion_1.BaseMovement {
    ...
    async execActionWithCount(position, vimState, count) {
        vimState.currentRegisterMode = register_1.RegisterMode.LineWise;
        return textEditor_1.TextEditor.getFirstNonWhitespaceCharOnLine(position.getUp(Math.max(count, 1)).line);
    }
};
...
let MoveDownUnderscore = class MoveDownUnderscore extends baseMotion_1.BaseMovement {
    ...
    async execActionWithCount(position, vimState, count) {
        return textEditor_1.TextEditor.getFirstNonWhitespaceCharOnLine(position.getDown(Math.max(count - 1, 0)).line);
    }
};
 ...
 */
function findHelper(start, char, count, direction) {
    const line = textEditor_1.TextEditor.getLineAt(start);
    ...
    if (index >= 0) {
        return new position_1.Position(start.line, index);
    }
    return undefined;
}
...
function tilHelper(start, char, count, direction) {
    const result = findHelper(start, char, count, direction);
    return direction === 'forward' ? result === null || result === void 0 ? void 0 : result.getLeft() : result === null || result === void 0 ? void 0 : result.getRight();
}
...
let MoveNonBlank = class MoveNonBlank extends baseMotion_1.BaseMovement {
    ...
    async execAction(position, vimState) {
        return textEditor_1.TextEditor.getFirstNonWhitespaceCharOnLine(position.line);
    }
};
...
let MoveNextLineNonBlank = class MoveNextLineNonBlank extends baseMotion_1.BaseMovement {
    ...
    async execActionWithCount(position, vimState, count) {
        ...
        return textEditor_1.TextEditor.getFirstNonWhitespaceCharOnLine(position.getDown(count).line);
    }
};
...
let MoveNonBlankFirst = class MoveNonBlankFirst extends baseMotion_1.BaseMovement {
    ...
    async execActionWithCount(position, vimState, count) {
        const lineNumber = util_1.clamp(count, 1, textEditor_1.TextEditor.getLineCount()) - 1;
...
let MoveWordBegin = class MoveWordBegin extends baseMotion_1.BaseMovement {
    ...
    async execAction(position, vimState, isLastIteration = false) {
        if (isLastIteration &&
...
class MoveSectionBoundary extends baseMotion_1.BaseMovement {
    ...
    async execAction(position, vimState) {
        if ((this.forward && position.line === textEditor_1.TextEditor.getLineCount() - 1) ||
        ...
        while (!textEditor_1.TextEditor.getLineAt(position).text.startsWith(this.boundary)) {
            if (this.forward) {
                if (position.line === textEditor_1.TextEditor.getLineCount() - 1) {
                    break;
                }
...
let MoveToMatchingBracket = MoveToMatchingBracket_1 = class MoveToMatchingBracket extends baseMotion_1.BaseMovement {
    ...
    async execActionWithCount(position, vimState, count) {
        ...
        if (Object.getPrototypeOf(this) === MoveToMatchingBracket_1.prototype) {
            ...
            return textEditor_1.TextEditor.getFirstNonWhitespaceCharOnLine(targetLine - 1);
        }
        else {
