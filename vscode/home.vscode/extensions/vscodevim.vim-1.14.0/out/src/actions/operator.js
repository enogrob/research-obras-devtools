...
class BaseOperator extends base_1.BaseAction {
    ...
    runRepeat(vimState, position, count) {
        ...
        return this.run(vimState, position.getLineBegin(), position.getDown(Math.max(0, count - 1)).getLineEnd());
    }
...
let DeleteOperator = class DeleteOperator extends BaseOperator {
     ...
     */
    async delete(start, end, currentMode, registerMode, vimState, yank = true) {
    ...
    async run(vimState, start, end, yank = true) {
...
let YankOperator = class YankOperator extends BaseOperator {
    ...
    async run(vimState, start, end) {
        ...
        if (vimState.surround) {
            ...
            await vimState.setCurrentMode(mode_1.Mode.SurroundInputMode);
...
let FormatOperator = class FormatOperator extends BaseOperator {
    ...
    async run(vimState, start, end) {
        ...
        let newCursorPosition = textEditor_1.TextEditor.getFirstNonWhitespaceCharOnLine(line);
...
let UpperCaseOperator = class UpperCaseOperator extends BaseOperator {
    ...
    async run(vimState, start, end) {
        ...
        await textEditor_1.TextEditor.replace(range, text.toUpperCase());
        await vimState.setCurrentMode(mode_1.Mode.Normal);
        vimState.cursorStopPosition = start;
        return vimState;
    }
...
let UpperCaseVisualBlockOperator = class UpperCaseVisualBlockOperator extends BaseOperator {
    ...
    async run(vimState, startPos, endPos) {
        ...
        const cursorPosition = position_1.Position.earlierOf(startPos, endPos);
...
let LowerCaseOperator = class LowerCaseOperator extends BaseOperator {
    ...
    async run(vimState, start, end) {
        const range = new vscode.Range(start, new position_1.Position(end.line, end.character + 1));
        let text = vimState.editor.document.getText(range);
        await textEditor_1.TextEditor.replace(range, text.toLowerCase());
        await vimState.setCurrentMode(mode_1.Mode.Normal);
        vimState.cursorStopPosition = start;
        return vimState;
    }
...
let OutdentOperator = class OutdentOperator extends BaseOperator {
    ...
    async run(vimState, start, end) {
        ...
        await vimState.setCurrentMode(mode_1.Mode.Normal);
        vimState.cursorStopPosition = textEditor_1.TextEditor.getFirstNonWhitespaceCharOnLine(start.line);
        return vimState;
    }
};
...
let ChangeOperator = class ChangeOperator extends BaseOperator {
    ...
    async run(vimState, start, end) {
        ...
        if (textEditor_1.TextEditor.getLineLength(textEditor_1.TextEditor.getLineAt(start).lineNumber) !== 0 ||
            ...
            if (isLineWise) {
                vimState = await new DeleteOperator(this.multicursorIndex).run(vimState, start.getLineBegin(), end.getLineEnd().getLeftThroughLineBreaks(), false);
            }
            else if (isEndOfLine) {
    ...
    async runRepeat(vimState, position, count) {
        ...
        vimState.currentRegisterMode = register_1.RegisterMode.LineWise;
        vimState = await this.run(vimState, position.getLineBegin(), position.getDown(Math.max(0, count - 1)).getLineEnd());
...
let ToggleCaseOperator = ToggleCaseOperator_1 = class ToggleCaseOperator extends BaseOperator {
    ...
    async run(vimState, start, end) {
        ...
        await ToggleCaseOperator_1.toggleCase(range);
        const cursorPosition = position_1.Position.earlierOf(start, end);
        vimState.cursorStopPosition = cursorPosition;
        vimState.cursorStartPosition = cursorPosition;
        await vimState.setCurrentMode(mode_1.Mode.Normal);
        return vimState;
    }
    static async toggleCase(range) {
        ...
        await textEditor_1.TextEditor.replace(range, newText);
    }
};
