...
class Register {
    ...
    static putByKey(content, register = '"', registerMode = RegisterMode.AscertainFromCurrentMode, force = false) {
        if (!Register.isValidRegister(register)) {
            throw new Error(`Invalid register ${register}`);
        }
        if (Register.isClipboardRegister(register)) {
     ...
     * register ".
     */
    static async get(vimState) {
        const register = vimState.recordedState.registerName;
        return Register.getByKey(register, vimState);
    }
    static async getByKey(register, vimState) {
        if (!Register.isValidRegister(register)) {
            throw new Error(`Invalid register ${register}`);
        }
        ...
        if (Register.isClipboardRegister(register)) {
            let text = await clipboard_1.Clipboard.Paste();
