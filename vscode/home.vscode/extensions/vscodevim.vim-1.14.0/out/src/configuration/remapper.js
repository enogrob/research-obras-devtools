...
class Remappers {
    ...
    async sendKey(keys, modeHandler, vimState) {
        let handled = false;
...
class Remapper {
    ...
    async handleRemapping(remapping, vimState, modeHandler) {
        ...
        if (vimState.currentMode === mode_1.Mode.Insert) {
            ...
            await vimState.historyTracker.undoAndRemoveChanges(Math.max(0, numCharsToRemove * vimState.cursors.length));
        ...
        if (remapping.commands) {
            for (const command of remapping.commands) {
                ...
                if (commandString.slice(0, 1) === ':') {
                    ...
                    await commandLine_1.commandLine.Run(commandString.slice(1, commandString.length), modeHandler.vimState);
