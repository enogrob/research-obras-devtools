...
exports.getCompletionsForCurrentLine = (position, document) => {
    const currentLineText = textEditor_1.TextEditor.getText(new vscode.Range(textEditor_1.TextEditor.getFirstNonWhitespaceCharOnLine(position.line), position));
...
exports.lineCompletionProvider = {
    ...
    showLineCompletionsQuickPick: async (position, vimState) => {
        ...
        vimState.recordedState.transformations.push({
            type: 'deleteRange',
            range: new range_1.Range(textEditor_1.TextEditor.getFirstNonWhitespaceCharOnLine(position.line), position.getLineEnd()),
        });
        vimState.recordedState.transformations.push({
